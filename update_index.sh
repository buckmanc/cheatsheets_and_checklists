...
baseToc="$(tree docs/ --prune --noreport -H 'docs/' | perl -00pe 's/(^.+<\/h1><p>|<hr>.+$)//sg' | sed 's/[└─├│ ]/ /g' | sed 's/^\t    //g' | grep -Piv 'index(_template)?\.md' | grep -Piv '^\s*$' | tail -n +2 | perl -pe 's/^( *)/$1- /g' | perl -pe 's|/+|/|g' | perl -pe 's|\.md(</a>)|$1|g' | perl -pe 's|<a href.+?/">([^<]+)</a>|$1|g')"
